...
extern int xwr3100_nvram_set(const char *name, const char *value);
extern void xwr3100_nvram_patch();
...
void xwr3100_init()
{
	...
	_dprintf("############################ MerlinR init nvram #################################\n");
	if (!nvram_get("bkmacaddr"))
		nvram_set("bkmacaddr", cfe_nvram_get("et0macaddr"));
	strcpy(mac1,nvram_get("bkmacaddr"));
	ether_atoe(mac1, mac_binary);
	ether_atoe(mac1, mac_binary2);
	strcpy(mac2,nvram_get("bkmacaddr"));
	mac_binary2[5] = mac_binary[5] +4;
	ether_etoa(mac_binary2, mac3);
	if (!nvram_get("bkmacaddr0"))
		nvram_set("bkmacaddr0", mac2);
	if (!nvram_get("bkmacaddr1"))
		nvram_set("bkmacaddr1", mac3);
...
void xwr3100_init_done(){
	...
	if(!cfe_nvram_get("uuid")){
		...
		xwr3100_nvram_set("uuid",nvram_get("uuid"));
		sleep(2);
	} else
		nvram_set("uuid",cfe_nvram_get("uuid"));

	if(!cfe_nvram_get("territory_code") || !strcmp(cfe_nvram_get("territory_code"), "US/01")){
		xwr3100_nvram_set("territory_code","US/01");
		nvram_set("location_code", "US");
	}
	logmessage("XWR3100", "uuid:%s", nvram_get("uuid"));
	logmessage("XWR3100", "mac:%s", cfe_nvram_get("et0macaddr"));
...
#if defined(K3) || defined(K3C) || defined(R8000P) || defined(R7900P) || defined(XWR3100)
...
#endif
	{
		logmessage("XWR3100", "*** verify done ***\n");
	} else {
