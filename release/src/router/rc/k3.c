...
void k3_init()
{
	...
	if (!nvram_get("k3macaddr1"))
		nvram_set("k3macaddr1", nvram_get("2:macaddr"));
	...
	if (nvram_get("2:pa5g40a0") || nvram_get("2:pa5g40a1")){
		...
		//macp = nvram_get("k3macaddr");
		//macp2 = nvram_get("k3macaddr1");
		//ether_atoe(macp, mac_binary);
		//ether_atoe(macp2, mac_binary2);
		////mac_binary[5]=mac_binary[5] +1;
		//mac_binary2[5]=mac_binary2[5] +1;
		//memset(macaddr1, 0, sizeof(macaddr1));
		//memset(macaddr2, 0, sizeof(macaddr2));
		//sprintf(macaddr1, "%02X:%02X:%02X:%02X:%02X:%02X", mac_binary[0], mac_binary[1], mac_binary[2], mac_binary[3], mac_binary[4], mac_binary[5]);
		//sprintf(macaddr2, "%02X:%02X:%02X:%02X:%02X:%02X", mac_binary2[0], mac_binary2[1], mac_binary2[2], mac_binary2[3], mac_binary2[4], mac_binary2[5]);
	if (strcmp(nvram_safe_get("0:macaddr"), nvram_get("k3macaddr")) || strcmp(nvram_safe_get("1:macaddr"), nvram_get("k3macaddr1")) || strcmp(nvram_get("devpath0"), "pcie/1/1/") || strcmp(nvram_get("devpath1"), "pcie/2/1/")) {
		nvram_set("0:macaddr", nvram_get("k3macaddr"));
	...
	//if (strcmp(nvram_safe_get("2:macaddr"), nvram_safe_get("k3macaddr1"))!=0)//3100 use 0 and 1
		...
		nvram_set("wl0_hwaddr", nvram_get("k3macaddr"));
