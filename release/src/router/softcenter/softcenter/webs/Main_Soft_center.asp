...
function appPostScript(moduleInfo, script) {
    ...
    data["softcenter_home_url"] = "https://scarmb.paldier.com";
...
<script>
    ...
    db_softcenter_["softcenter_home_url"] = "https://scarmb.paldier.com";
    ...
    function init(cb) {
        ...
        function _mergeData(remoteData) {
            ...
            $.map(result, function (item, name) {
                ...
                // 如果是未安装的插件,则必定在 https://scarmb.paldier.com/{name}/{name}/icon-{name}.png
                // TODO 如果因为一些错误导致没有图标, 有可能显示一张默认图标吗?
                item.icon = parseInt(item.install, 10) !== 0
                    ? ('/res/icon-' + item.name + '.png')
                    : ('https://scarmb.paldier.com' + new Array(3).join('/softcenter') + '/res/icon-' + item.name + '.png');
            });
            return result;
        };
...
function notice_show(){
    $.ajax({
        url: 'https://scarmb.paldier.com/softcenter/push_message.json.js',
        type: 'GET',
        dataType: 'jsonp',
